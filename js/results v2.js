...
const Results = (() => {
  ...
  async function analyze(session) {
    ...
    const students = [...ids].sort((a, b) => a - b).map(id => {
      ...
      return {
        ...
        label: (e.prenom || e.nom) ? [e.prenom, e.nom].filter(Boolean).join(' ') : I18n.t('scan.card', { id: id }),
  ...
  async function render(selectId) {
    ...
    sessions.forEach(s => {
      ...
      opt.value = s.id;
      opt.textContent = (s.quizTitre || s.quizId) + ' \u2014 ' + new Date(s.date).toLocaleString(I18n.locale());
  ...
  async function analyzeSelected() {
    ...
    if (session && !data) {
      errEl.textContent = I18n.t('results.quizMissing', { title: session.quizTitre || session.quizId });
  ...
  function cellMark(a, question, student) {
    let reponse;
    if (a.letter === null) {
      reponse = I18n.t('results.answerNone');
    } else {
      const idx = LETTERS.indexOf(a.letter);
      const texte = question.choix[idx];
      reponse = a.letter + (texte ? ' \u2014 ' + MathText.plain(texte) : '');
    }
    const tip = I18n.t('results.tooltipQuestion', { text: MathText.plain(question.texte) }) + '\n' +
      I18n.t('results.tooltipAnswer', { name: student.label, answer: reponse });
    let mark;
  ...
  function renderSummary() {
    ...
    let html =
      '<div class="rc-stats">' +
        '<div class="rc-stat"><div class="rc-stat-label">' + I18n.t('results.statPrecision') + '</div>' +
          '<div class="rc-stat-value ' + pctClass(precision) + '">' +
          (precision === null ? '\u2013' : precision + '\u00a0%') + '</div></div>' +
        '<div class="rc-stat"><div class="rc-stat-label">' + I18n.t('results.statCompletion') + '</div>' +
          '<div class="rc-stat-value ' + pctClass(achevement) + '">' +
          (achevement === null ? '\u2013' : achevement + '\u00a0%') + '</div></div>' +
        '<div class="rc-stat"><div class="rc-stat-label">' + I18n.t('results.statStudents') + '</div>' +
          '<div class="rc-stat-value">' + N + '</div></div>' +
        '<div class="rc-stat"><div class="rc-stat-label">' + I18n.t('results.statQuestions') + '</div>' +
          '<div class="rc-stat-value">' + n + '</div></div>' +
      '</div>';

    html += '<div class="rc-sortbar"><label for="rc-summary-sort">' + I18n.t('results.sortBy') + '</label>' +
      '<select id="rc-summary-sort">' +
      '<option value="nom">' + I18n.t('results.sortLastName') + '</option>' +
      '<option value="prenom">' + I18n.t('results.sortFirstName') + '</option>' +
      '<option value="precision">' + I18n.t('results.statPrecision') + '</option>' +
      '</select></div>';
